...
trans_Extra_diff_result<-function(df, 
  ...
  #合并差异结果以及表达注释文件夹
  trans_anno_count_rpkm<-df %>% inner_join(trans_DEGs_res,"transcript_id")
  
  save(trans_anno_count_rpkm,file=file.path(combat_dir,comp_filename,dir,"All_Trans","trans_anno_count_rpkm.Rdata")) 
  fwrite(trans_anno_count_rpkm,file=file.path(combat_dir,comp_filename,dir,"All_Trans","trans_anno_count_rpkm.txt"),row.names = F,quote = F,sep = "\t")
  ...
  All_result<-trans_anno_count_rpkm %>%as.data.frame() %>%  dplyr::select(all_of(col_in)) %>% as.data.frame() 
