...
#rpkm文件
rpkm_raw_df<-read_csv("01_Raw_Data/ref_rpkm/gene_ref_rpkm.csv") %>% 
             tidyr::separate(col = gene_id, into = c("gene_id", "gene_name"), sep = "\\|", remove = TRUE) %>% 
             rename_at(3:ncol(.),list(~ str_glue("{.}_rpkm"))) %>% 
             select(1,3:ncol(.))

#注释文件，count文件，rpkm文件合并
anno_count_rpkm<- annotation %>% 
  inner_join(count_raw_df, by="gene_id") %>% 
  inner_join(rpkm_raw_df, by="gene_id") %>% 
  dplyr::distinct(gene_name, .keep_all = TRUE)

save(anno_count_rpkm,file="01_Raw_Data/anno_count_rpkm.RData")
load("01_Raw_Data/anno_count_rpkm.Rdata")
...
PCA_new(anno_count_rpkm[,paste0(Group$Sample,"_rpkm")],
  ntop = 500,
  group = Group$Group,
  show_name = T)
dev.off()
...
boxplot(anno_count_rpkm[,paste0(Group$Sample,"_rpkm")],
  las = 2,
  outline = FALSE,
...
# adjusted_count<-Combat_count(Group,anno_count_rpkm)
# 
# #将原始数据中的count值进行替换
# anno_count_rpkm[,match(paste0(Group$Sample,"_count"),colnames(anno_count_rpkm))]<-adjusted_count
# 
# 
# #04将矫正后的count值进行rpkm值的转换------------------------------------------
# 
# #载入转化函数
# source("00_Code/Count_to_rpkm.R",encoding = "utf-8")
# 
# rpkms<-Count_to_rpkm(anno_count_rpkm,gene_length,Group)
# 
# #将rpkms中的gene_id按照我们的anno_count_rpkm 进行排序
# rpkms<- rpkms[match( anno_count_rpkm$gene_id , rpkms$gene_id ),]
# rownames(rpkms)<-rpkms$gene_id
# rpkms<-rpkms[,-1]
# 
# #将原始数据中的rpkm值进行替换
# anno_count_rpkm[,match(paste0(Group$Sample,"_rpkm"),colnames(anno_count_rpkm))]<-rpkms
...
DEGs_res<-Diff_edgeR(anno_count_rpkm,Group)
# DEGs_res<-Diff_DESeq2(anno_count_rpkm,Group)
...
#rpkm文件
trans_rpkm_raw_df<-read_csv("01_Raw_Data/ref_rpkm/transcript_ref_rpkm.csv") %>% 
  rename_at(2:ncol(.),list(~ str_glue("{.}_rpkm"))) %>% 
  select(1,2:ncol(.))

#注释文件，count文件，rpkm文件合并
trans_anno_count_rpkm<- annotation_trans %>% 
  inner_join(trans_count_raw_df, by="transcript_id") %>% 
  inner_join(trans_rpkm_raw_df, by="transcript_id") %>% 
  dplyr::distinct(transcript_id, .keep_all = TRUE)

save(trans_anno_count_rpkm,file="01_Raw_Data/trans_anno_count_rpkm.RData")
load("01_Raw_Data/trans_anno_count_rpkm.Rdata")

source("00_Code/Trans_diff_edgeR.R", encoding = "utf-8")
# edgeR 
#分组的文件表

trans_DEGs_res<-Diff_edgeR(trans_anno_count_rpkm,Group)
