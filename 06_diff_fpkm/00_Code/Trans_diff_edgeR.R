library(edgeR)
# 函数说明：edger差异分析、输入：样本矩阵，矩阵中包含样本名字以及transcript_id,以及分组的文件Group
Diff_edgeR<-function(data,Group){
  ...
  Treat_Control<-trans_anno_count_fpkm[,c("transcript_id",Treat,Control)] %>%
    as.data.frame() %>% 
  ...
  trans_DEGs_res <- as.data.frame(topTags(dge.list.res,n=nrow(Treat_Control),sort.by = "logFC"))
  trans_DEGs_res<-data.frame(transcript_id=rownames(trans_DEGs_res),trans_DEGs_res)
  
  trans_DEGs_res<-trans_DEGs_res[,c(1,2,4,5)]
  return(trans_DEGs_res)
}
